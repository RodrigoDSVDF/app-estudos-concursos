...
const BackupView = {
  ...
  async exportJson() {
    ...
    a.href = url;
    a.download = `concursotrack_backup_${Utils.todayISO()}.json`;
    a.click();
  ...
  async exportXlsxAll() {
    ...
    XLSX.writeFile(wb, `concursotrack_completo_${Utils.todayISO()}.xlsx`);
  ...
  async exportPdf() {
    ...
    doc.text('ConcursoTrack — Relatório de Estudos', 40, 45);
    doc.setFontSize(10);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `ConcursoTrack — página ${i} de ${pageCount}`,
        pageWidth / 2,
    ...
    doc.save(`relatorio_concursotrack_${Utils.todayISO()}.pdf`);
